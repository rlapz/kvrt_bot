...
config_dump(const Config *c)
{
	...
	printf("DB session path            : %s\n", c->db_session_path);
	printf("DB session pool connections: %d\n", c->db_session_pool_conn_size);
...
_parse_json_sys(Config *c, json_object *root_obj)
{
	...
	const char *db_session_file = CFG_DEF_SYS_DB_SESSION_PATH;
	uint16_t db_session_pool_conn_size = CFG_DEF_DB_SESSION_CONN_POOL_SIZE;
	...
	if (json_object_object_get_ex(sys_obj, "db_session_file", &tmp_obj) != 0) {
		const char *const _db_file = json_object_get_string(tmp_obj);
		if (cstr_is_empty(_db_file) == 0)
			db_session_file = _db_file;
	}

	if (json_object_object_get_ex(sys_obj, "db_session_pool_conn_size", &tmp_obj) != 0) {
		const int sz = json_object_get_int(tmp_obj);
		if (sz > 0)
			db_session_pool_conn_size = sz;
	}
