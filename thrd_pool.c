...
static int          _jobs_enqueue(ThrdPool *t, ThrdPoolFn func, void *udata0, void *udata1);
...
thrd_pool_destroy(ThrdPool *t)
{
	...
	if (t->threads != NULL) {
		for (unsigned i = 0; i < t->workers_len; i++) {
			if (thrd_join(t->threads[i], NULL) != 0)
...
int
thrd_pool_add_job(ThrdPool *t, ThrdPoolFn func, void *udata0, void *udata1)
{
	int ret = -1;
	mtx_lock(&t->mtx_general); /* LOCK */

	ret = _jobs_enqueue(t, func, udata0, udata1);
	if (ret < 0) {
...
static int
_jobs_enqueue(ThrdPool *t, ThrdPoolFn func, void *udata0, void *udata1)
{
...
_worker_fn(void *udata)
{
	...
	while (t->is_alive) {
		...
		tmp_func = job->func;
		tmp_udata0 = job->udata0;
		tmp_udata1 = job->udata1;

		mtx_unlock(&t->mtx_general); /* UNLOCK */

		assert(tmp_func != NULL);
		tmp_func(tmp_udata0, tmp_udata1);
