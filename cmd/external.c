...
static void _spawn_child_process(Update *u, const TgMessage *msg, json_object *json, const DbCmd *dbcmd);
...
cmd_external_exec(Update *u, const TgMessage *msg, const BotCmd *cmd, json_object *json)
{
	...
	if (dbcmd.is_enable == 0)
		return 1;

	if ((dbcmd.is_admin_only != 0) && (common_privileges_check(u, msg) < 0))
		return 1;

	_spawn_child_process(u, msg, json, &dbcmd);
	return 1;
}
